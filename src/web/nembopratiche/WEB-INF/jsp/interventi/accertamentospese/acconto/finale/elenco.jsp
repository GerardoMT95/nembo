...
<c:if test="${msgIVA!=null}">
				...
				<table summary="dettaglio" id='tblInterventi' summary="Elenco Interventi" class="table table-hover table-bordered tableBlueTh show-totali" data-toggle="table"
					...
					<thead>
						<tr>
							...
							<th data-field="spesaRiconosciutaPerCalcolo" data-totale="true" data-formatter="numberFormatter2" data-adder="importoAdder">Spesa<br />riconosciuta<br/>per il calcolo<br/>del contributo
							</th>
			               <th data-field="importoNonRiconosciuto" data-totale="true" data-formatter="numberFormatter2" data-adder="importoAdder" data-visible="true">Importo<br/>non<br/>riconosciuto<br/>sanzionabile
			                ...
			                <th data-field="importoRispendibile" data-totale="true" data-formatter="numberFormatter2" data-adder="importoAdder" data-visible="true">Importo non<br/>riconosciuto<br/>non sanzionabile
			                </th>
        ...
        <m:panel id="totaliContributo_${status.index}" title="${contributo.codiceOperazione}">
          ...
          <tr><th>Contributo erogabile</th><td style="padding-left:20px;min-width:160px" class="numero"><fmt:formatNumber value="${contributo.contributoErogabile}" pattern="###,##0.00"/> &euro;</td></tr>
...
title='Compensazione per arrotondamenti rispetto alla colonna "Contributo calcolato" o alla colonna nascosta "Contributo abbattuto"'">
          ...
          <td style="padding-left: 20px; min-width: 160px" class="numero"><fmt:formatNumber value="${contributo.compensazioneArrotondamenti}" pattern="###,##0.00" /> &euro;</td></tr>
          </c:if>
          <tr><th>Contributo non erogabile</th><td style="padding-left:20px" class="numero"><fmt:formatNumber value="${contributo.contributoNonErogabile}" pattern="###,##0.00"/> &euro;</td></tr>
          <tr><th>Di cui totale sanzioni/riduzioni</th><td style="padding-left:20px" class="numero"><fmt:formatNumber value="${contributo.importoSanzioni}" pattern="###,##0.00"/> &euro;</td></tr>
          </table>
        </m:panel>
        </c:forEach>
				<br />
    ...
    function visualizzainterventi(idIntervento)
    {
    	$.ajax({
            ...
            success: function(data) {
            	$('tbody input[value='+idIntervento+']').closest('tr').after('<tr id=\"dettaglioimporti_'+idIntervento+'\"><td colspan=\"16\">'+data+'</td></tr>');
				$('#espandi_'+idIntervento).html('<a style="text-decoration: none;font-size:18px;vertical-align:middle;margin-bottom:1em;" class="glyphicon glyphicon-minus-sign"  href=\"#'+idIntervento+'\" onclick=\"nascondiinterventi('+idIntervento+');\"></a>')
    ...
    function nascondiinterventi(idIntervento)
    {
    	...
    	$('#espandi_'+idIntervento).html('<a style="text-decoration: none;font-size:18px;vertical-align:middle;margin-bottom:1em;" class="glyphicon glyphicon-plus-sign"  href=\"#'+idIntervento+'\" onclick=\"visualizzainterventi('+idIntervento+');\"></a>')
    }
