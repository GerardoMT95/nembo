...
<body>
	...
	<form name="mainForm" method="post" action="${action}">
		<div class="container-fluid" id="content">
			...
			<c:forEach items="${interventi.values()}" var="intervento">
				...
				<m:panel id="panelModificaRendicontazioneDocumenti_${intervento.idIntervento}" title="${intervento.descrizioneEstesa}">
					...
					<table summary="dettaglio" id='rendicontazioneDocumenti' summary="Elenco Interventi" class="table table-hover table-bordered tableBlueTh show-totali" data-toggle="no-toggle"
						...
						<thead>
							<tr>
								...
								<th data-totale="true" data-switchable="false" data-field="importo"  data-formatter="numberFormatter2" data-adder="plainNumberAdder">Importo<br />associato<br />documento
								</th>
                <th data-totale="true" data-switchable="false" data-field="importoRicevutePagamento" data-adder="plainNumberAdder" data-formatter="numberFormatter2">Importo<br />associato<br/>ricevute
                </th>
								...
								<th data-totale="true" data-totale-formatter="numberFormatter2" data-switchable="false" data-field="importoRendicontatoCorrente" data-formatter="inputFormatter">Importo<br />in<br/>rendicontazione
								</th>
	...
	<div style="display: none" id="invisibleTextfield">
		<div class=":CSS" data-original-title=":ERROR" data-toggle=":TOGGLE" style="min-width: 140px">
			<m:textfield id=":ID" name=":NAME" type="EURO" value=":VALUE" onkeyup=":ONKEYUP" onchange=":ONCHANGE"/>
		</div>
	</div>
  ...
  function calcolaImportoRendicontato()
  {
    ...
    function inputFormatter($value, row, index)
    {
      ...
      if (row['errorMessage'])
      {
        html = html.replace(":CSS", 'importo_rendicontato has-error red-tooltip');
    ...
    function initFilter(id)
    {
      ...
      $('#filter-bar_'+id).bootstrapTableFilter(
      {
        ...
        //connectTo : '#rendicontazioneDocumenti_'+id
      });
    }
