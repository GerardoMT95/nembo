...
!function ($) {
    ...
    let calculateObjectValue = function (self, name, args, defaultValue) {
        ...
        if (typeof func === 'function') {
        	if(func.toString().indexOf("esclusioneFormatter")>-1) //queste due righe aggiunte per gestire il formatter in CUPSR20196-197
        		args[0] = self.field;							  //Non ho trovato altro modo per passare al formatter il field chiamante.
    ...
    BootstrapTable.DEFAULTS = {
        ...
        searchTimeOut: 500,
        searchText: '',
        escapeTable: undefined,
        iconSize: undefined,
        iconsPrefix: 'glyphicon', // glyphicon of fa (font awesome)
        icons: {
            paginationSwitchDown: 'glyphicon-collapse-down icon-chevron-down',
            paginationSwitchUp: 'glyphicon-collapse-up icon-chevron-up',
            defaultOrder: 'glyphicon-sort-by-attributes',
        ...
        onPageListChange: function (number, size) {
            return false;
        },
    ...
    $.extend(BootstrapTable.DEFAULTS, BootstrapTable.LOCALES['it-IT']);
    ...
    BootstrapTable.EVENTS = {
        ...
        'pageList-change.bs.table': 'onPageListChange',
    ...
    BootstrapTable.prototype.initContainer = function () {
        this.$container = $([
            ...
            this.options.formatLoadingMessage(), this.options.formatRefresh(),
            '</div>',
            '</div>',
        ...
        this.$header.find('tr').each(function () {
            ...
            $(this).find('th').each(function () {
                column.push($.extend({}, {
                    ...
                    switchGroup: $(this).data('switchGroup') ? +$(this).data('switchGroup') : undefined,
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        that.options.columnsWidthGroup=[];
        ...
        $.each(this.options.columns, function (i, columns) {
            ...
            $.each(columns, function (j, column) {
                ...
                if(column.switchGroup)
                {
                	column.rowIndex = i;
                	column.colIndex = j;
                	that.options.columnsWidthGroup.push(column); 
                }
                ...
                if(column.escapeField === undefined) {
                	column.escapeField = that.options.escapeTable;
                }
                ...
                html.push('<th' + sprintf(' title="%s"', column.titleTooltip),
                    ...
                    sprintf(' data-totale="%s"', column.totale),
                    
                    column.switchGroup ? sprintf(' data-switch-group="%s"', column.switchGroup) : '',
                    column.switchGroupLabel ? sprintf(' data-switch-group-label="%s"', column.switchGroupLabel) : '',
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.showDefaultOrderButton && this.options.defaultOrderColumn && this.options.defaultOrderType) {
            html.push('<button id="defaultOrderButton" class="btn btn-default" type="button" name="defaultOrderButton" style="text-decoration: none;vertical-align:center;margin-bottom:2em;color:black;" title="Ripristino ordinamento iniziale" onclick="resetDefaultOrder(\''+this.options.tableName+'\', \''+this.options.defaultOrderColumn+'\', \''+this.options.defaultOrderType+'\');">');
            html.push(sprintf('<i class="%s %s"></i>', this.options.iconsPrefix, this.options.icons.defaultOrder),
                '</button>');
        }
        ...
        if (this.options.showColumns) {
            ...
            $.each(this.columns, function (i, column) {
                ...
                if (column.switchGroupLabel) {
                	...
                	if($.inArray(column.switchGroup, testo) == -1)
                	{
                		testo.push(column.switchGroup);
                    	html.push(sprintf('<li>' +
                                '<label><input type="checkbox" data-group="%s" value="%s"%s> %s</label>' +
                                '</li>', column.switchGroup, i, checked, column.switchGroupLabel.replace('<br>',' ')));
                	}
                    switchableCount++;
                }
                else if (!column.switchGroup && column.switchable) {
                    html.push(sprintf('<li>' +
        ...
        if (this.options.showColumns) {
            ...
            $keepOpen.find('input').off('click').on('click', function () {
                ...
                }else if($(this).data('group'))
                {
                    ...
                    that.initPagination();
                    that.initBody();
                	that.trigger('column-switch', $(this).data('group'), $this.prop('checked'));
                }
            });
        }
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        html.push(
                '<div class="pull-' + this.options.paginationDetailHAlign + ' pagination-detail">',
                '<span class="pagination-info">',
                this.options.formatShowingRows(this.options.pageNumber, this.totalPages, this.options.totalRows),
                '</span>');
    ...
    BootstrapTable.prototype.onPageListChange = function (event) {
        ...
        this.trigger('pageList-change', this.options.pageSize);
                ...
                if (column.checkbox || column.radio) {
                    ...
                    text = that.options.cardView ? ['<div class="card-view">',
                        ...
                        '</div>'
                         ...
                         that.$header.find('th[data-field="'+field+'"]').data('switch-group') ?          
                         sprintf('<td   %s %s %s %s %s %s data-field="%s" data-switch-group="%s">', id_, class_, style, data_, rowspan_, title_,field, that.$header.find('th[data-field="'+field+'"]').data('switch-group'))
                         :
                         sprintf('<td   %s %s %s %s %s %s data-field="%s">', id_, class_, style, data_, rowspan_, title_,field)
                         ,
    ...
    BootstrapTable.prototype.toggleColumnRow = function (rowHeader, index, checked, needUpdate) {
        if (index === -1) {
            return;
        }
        this.options.columns[rowHeader][index].visible = checked;
        this.initHeader();
        this.initSearch();
        this.initPagination();
        this.initBody();
...
!function(a){"use strict";a.fn.bootstrapTable.locales["it-IT"]={formatLoadingMessage:function(){return    showPleaseWait()    },formatRecordsPerPage:function(a){return a+" records per pagina"},formatShowingRows:function(a,b,c){return"Pagina "+a+" di "+b+" ("+c+" records totali)"},formatSearch:function(){return"Cerca"},formatNoMatches:function(){return"Nessun record trovato"},formatRefresh:function(){ return showPleaseWait();},formatToggle:function(){return"Alternare"},formatColumns:function(){return"Colonne"}},a.extend(a.fn.bootstrapTable.defaults,a.fn.bootstrapTable.locales["it-IT"])}(jQuery);
$('body').on('load-success.bs.table', function()
    ...
    $('body').on('column-switch.bs.table', function()
    {
      aggiornaTotali();
    });
    ...
    function aggiornaTotali()
    {
      ...
      $(".show-totali").each(function()
      {
        ...
        $('#'+tableID+' th[data-totale="true"]').each(function () {
          ...
          if (__dataFormatter)
          {
            sum = eval(__dataFormatter+'('+sum+', rows, null)');
          }
     ...
     * Modifica per semplificare l'utilizzo delle table in tutto il progetto.
     * Richiamando questa funzione viene automaticamente impostato il tutto per salvare in sessione
     * e tenere in memoria il numero di pagina, la dimensione della pagina, le colonne visibili/nascoste, l'ordinamento 
     * e i filtri.
     * 
     * Necessario: 
     * 	- Controller: GestioneSessioneController.java
     *  - <c:set var ="tableName"  value =" "/> - impostare questo nella jsp con nel value l'id della table
     *  - il filtro hidden deve avere id="filtro"
     *  
     *  La funzione riceve come parametro il tableName (id della table) e i filtri da visualizzare.
     */ 

	function loadBootstrapTable(tableName, filters){
		/*Imposto filtri e li salvo in sessione quando applicati*/		
		$('#filter-bar').bootstrapTableFilter({
          ...
          onSubmit: function() {
	              ...
	              $.ajax({
	              	  ...
	              	  data: "key="+tableName+"&filtro="+elabFilter,
		...
		/*Mantengo in sessione le info relative alle colonne visibili/nascoste*/
		$('body').on("column-switch.bs.table", function(obj, field, checked) {
			$.ajax({
				type : "POST",
				url : '../session/salvaColonna.do',
				data : "key="+tableName+"&field=" + field + "&value=" + checked,
				success : function(data) {
				}
			});
		});
		...
		$('body').on("page-change.bs.table", function(obj, page) {
			$.ajax({
            	  type: "POST",
            	  url: '../session/salvaNumeroPagina.do',
            	  data: "key="+tableName+"&page="+page,
            	  success: function( data ){}
            	});
		});

		/*Mantengo in sessione l'ordinamento dell'elenco*/
		$('body').on("sort.bs.table", function(obj, col, order) {
			$.ajax({
            	  type: "POST",
            	  url: '../session/salvaOrdinamento.do',
            	  data: "key="+tableName+"&col="+col+"&order="+order,
            	  success: function( data ){
            		  console.log(data);
            	  }
            	});
		...
		/*Mantengo in sessione la dimensione della pagina*/
		$('body').on("pageList-change.bs.table", function(obj, size) {
			$.ajax({
            	  type: "POST",
            	  url: '../session/salvaPageSize.do',
            	  data: "key="+tableName+"&size="+size,
            	  success: function( data ){}
            	});
		});
	 ...
	 * @param orderType
	 * 
	 * Il pulsante per ripristinare l'ordinamento di default, se impostato, è visibile solo
	 * se sono impostati anche il defaultOrderColumn (data-default-order-columnt) 
	 * e il defaultOrderType (data-default-order-type).
	 * 
	 */
	function resetDefaultOrder(tableName, defaultOrderColumn, defaultOrderType){

		/*Mantengo in sessione l'ordinamento dell'elenco*/
			$.ajax({
            	  type: "POST",
            	  url: '../session/rimuoviOrdinamento.do',
            	  data: "key="+tableName,
            	  success: function( data ){
            		  $('#'+tableName).bootstrapTable('refreshOptions', {
							sortName : defaultOrderColumn,
							sortOrder : defaultOrderType,
						});
            	  }
            	});			
	}