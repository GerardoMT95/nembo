...
@Controller
@NemboSecurity(value = "CU-NEMBO-1304-MS", controllo = NemboSecurity.Controllo.PROCEDIMENTO_OGGETTO)
@RequestMapping(value = "/cunembo1304ms")
public class CUNEMBO1304MSModificaASaldo extends BaseController
{
  @Autowired
    ...
    if (list != null)
    {
      for (RigaModificaMultiplaInterventiDTO intervento : list)
      {
        ...
        if (!intervento.getMisurazioneIntervento().get(0).isMisuraVisibile())
        {
          model.addAttribute("errore",
              "L'intervento \"" + intervento.getDescIntervento()
                  + "\" non � soggetto a misurazione, impossibile modificarlo");
          return true;
        }
      }
    }
