package it.csi.nembo.nembopratiche.presentation.quadro.controlliexpost;
...
@Controller
@NemboSecurity(value = "CU-NEMBO-291-M", controllo = NemboSecurity.Controllo.PROCEDIMENTO_OGGETTO)
@RequestMapping(value = "/cunembo291m")
public class CUNEMBO291MControlloExPost extends BaseController
{
  public static final String BASE_JSP_URL               = "controlliexpost/";
  @Autowired
  IQuadroEJB                 quadroEJB;

  public static final String ERRORE_TECNICO_NON_TROVATO = "Il tecnico indicato non � stato trovato in elenco";
  public static final String ERRORE_ESITO_NON_TROVATO   = "L'esito indicato non � tra quelli disponibili";
  ...
  {
    if (!NemboConstants.FLAGS.SI.equals(datiSpecifici.getFlagPreavviso()))
    {
    ...
    List<DecodificaDTO<Integer>> tipologiePreavviso = quadroEJB
        .getTabellaDecodifica("NEMBO_D_TIPOLOGIA_PREAVVISO", true);
    model.addAttribute("tipologiePreavviso", tipologiePreavviso);
    model.addAttribute("modalitaSelezione",
        CUNEMBO291DControlloExPost.getHtmlDecodificaModalitaSelezione(
      ...
      HttpServletRequest request) throws InternalUnexpectedException
  {
    ...
    if (NemboConstants.FLAGS.SI.equals(flagPreavviso))
    {
      controlliInLocoInvestDTO.setFlagPreavviso(NemboConstants.FLAGS.SI);
    ...
    {
      controlliInLocoInvestDTO.setFlagPreavviso(NemboConstants.FLAGS.NO);
    }
    String flagControllo = request.getParameter("flagControllo");
    if (NemboConstants.FLAGS.SI.equals(flagControllo))
    {
      controlliInLocoInvestDTO.setFlagControllo(NemboConstants.FLAGS.SI);
    }
    else
    {
      controlliInLocoInvestDTO.setFlagControllo(NemboConstants.FLAGS.NO);
    }
    ...
    else
    {
      ...
      if (!NemboConstants.FLAGS.SI.equals(flagControllo))
      {
        ...
        QuadroOggettoDTO qoContrLoco = po.findQuadroByCodiceQuadro(
            NemboConstants.QUADRO.CODICE.CONTROLLI_EX_POST);
        QuadroOggettoDTO qoContr = po.findQuadroByCodiceQuadro(
            NemboConstants.QUADRO.CODICE.CONTROLLI_AMMINISTRATIVI);
        if (qoContrLoco != null)
        {
      ...
      throws InternalUnexpectedException
  {
    ...
    final List<ControlloAmministrativoDTO> controlliAmministrativi = quadroEJB
        .getControlliAmministrativiExPost(idProcedimentoOggetto,
            NemboConstants.QUADRO.CODICE.CONTROLLI_EX_POST, list);
    ...
    {
      // Inserimento
      return "redirect:../cunembo291d/index.do";
    }
    else
    {
      model.addAttribute("preferRequest", Boolean.TRUE);
    ...
    if (isDataVerbale != isNumeroVerbale)
    {
      if (isDataVerbale)
      {
        errors.addError(nameNumeroVerbale,
            "Se � presente la data del verbale deve essere inserito anche il numero del verbale");
      }
      else
      {
        errors.addError(nameDataVerbale,
            "Se � presente il numero del verbale deve essere inserita anche la data verbale");
      }
    }
    else
    {
