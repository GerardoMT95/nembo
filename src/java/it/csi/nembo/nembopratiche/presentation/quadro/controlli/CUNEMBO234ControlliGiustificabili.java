...
@Controller
@RequestMapping("/cunembo234")
@NemboSecurity(value = "CU-NEMBO-234-M", controllo = NemboSecurity.Controllo.PROCEDIMENTO_OGGETTO)
public class CUNEMBO234ControlliGiustificabili extends BaseController
{
  @Autowired
  ...
  @NemboSecurity(value = "CU-NEMBO-114", controllo = NemboSecurity.Controllo.DEFAULT)
      ...
      throws InternalUnexpectedException, IOException
  {
    ...
    if (idTipoRisoluzione != null)
    {
      ...
      if (flagFileObbligatorio.equals("S"))
      {
        ...
        // se non � modifica, ma inserimento, faccio il controllo che ci sia il
        // file
        if (!isModifica)
        {
        ...
        {
          // se � in modifica, non in inserimento, se il file esisteva gi�, ok,
      ...
      if (!isModifica)
      {
        // se sono in inserimento, se c'� il nome deve esserci anche il file e
        // viceversa
      ...
      {
        // se sono in modifica, se c'� il nome, o c'� il file o questo esisteva
        // gi�
        if (nomeAllegato != null && nomeAllegato.compareTo("") != 0)
        ...
        // se non c'� il file ma non il nome, errore
        if (fileAllegato != null && fileAllegato.getSize() != 0
            && (nomeAllegato == null || nomeAllegato.compareTo("") == 0))
          errors.addError("nomeAllegato",
              "E' stato inserito il file. Inserire anche il nome.");

        // se non c'� il nome ma c'� il file vero o preesistente
      ...
      if (errors.isEmpty())
      {
        if (!isModifica)
          quadroEJB.inserisciGiustificazioneAnomaliaControllo(
              ...
              NemboUtils.DATE.parseDate(dataProtocollo),
              NemboUtils.DATE.parseDate(dataDocumento), numProtocollo);
        else
        {
