...
public abstract class Modifica extends BaseController
{
      ...
      throws InternalUnexpectedException, ApplicationException
  {
    ...
    model.addAttribute("action", "../cunembo"
        + NemboUtils.APPLICATION.getCUNumber(request) + "m/modifica.do");
      ...
      throws InternalUnexpectedException
  {
    ...
    for(RigaModificaMultiplaInterventiDTO i : list){
    	if(i!=null && NemboConstants.PUNTEGGI.CODICE_INTERVENTI_PREVENZIONE.equalsIgnoreCase(i.getCodiceIdentificativo())){
    ...
    if (list != null)
    {
      for (RigaModificaMultiplaInterventiDTO i : list)
      {
        if (NemboConstants.FLAGS.SI.equals(i.getFlagBeneficiario()))
        {
  ...
  /*
   * Nota: Il metodo � riscritto nella classe CUNEMBO266MModificaInterventi,
   * mantenendo al contempo la logica "standard" e aggiungendo la validazione
   * delle attivit�/partecipanti, eventuali modifiche alla logica di base
      ...
      throws InternalUnexpectedException, ApplicationException
  {
    ...
    for (RigaModificaMultiplaInterventiDTO riga : list)
    {
      ...
      if (flagCostoUnitario)
      {
        ...
        {
          BigDecimal valore = misurazioni.get(0).getValore(); // C'� sempre
                                                              // almeno una
                                                              // misurazione per
                                                              // intervento
                                                              // (altrimenti la
          ...
          if (valore != null)
          {
            ...
            {
              final String errorMessage = "Il prodotto dell'importo unitario per la quantit� � superiore al massimo consentito (pari a "
      ...
      else
      {
        ...
        importo = errors.validateMandatoryBigDecimalInRange(
            ...
            BigDecimal.ZERO,
            NemboConstants.MAX.IMPORTO_INTERVENTO);
      }
      ...
      throws InternalUnexpectedException
  {
    ...
    BigDecimal bdPercentuale = errors.validateMandatoryBigDecimalInRange(
        ...
        BigDecimal.ZERO, NemboConstants.MAX.PERCENTUALE);
    ...
    if (errors.addToModelIfNotEmpty(model))
    {
      ...
      model.addAttribute("salvaPercentuale",
          NemboUtils.FORMAT.formatDecimal2(infoRiduzione.getPercentuale()));
