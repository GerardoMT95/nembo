...
@Controller
@NemboSecurity(value = "CU-NEMBO-214-M", controllo = NemboSecurity.Controllo.PROCEDIMENTO_OGGETTO)
@RequestMapping(value = "/cunembo214m")
public class CUNEMBO214MModificaRiduzioniSanzioni extends BaseController
{
      ...
      throws InternalUnexpectedException
  {
    ...
    for (Long id : ids)
    {
      ...
      // cerco nelle riduzionisanzioni gi� inserite e, se sono tra quelle che
      // sto modificando
      ...
      if (addErrorImporto)
        for (Long id : ids)
        {
          errors.addError("importo_" + id,
              "Il totale degli importi inseriti non pu� superare il valore: "
            ...
            && tipoPagamentoSigop.compareTo("SALDO") != 0))
    {
      ...
      model.addAttribute("msgErrore",
          "Si � verificato un errore della configurazione del quadro \"Riduzioni e sanzioni\", l'aggiornamento del contributo erogabile � previsto solo per le istruttorie delle domande di acconto e saldo. Non � possibile proseguire con l'operazione.");
    }
    ...
    return "redirect:../cunembo214l/index.do";
  }
      ...
      throws InternalUnexpectedException
  {
    ...
    // l'operazione � obbligatoria solo se l'idTipologia non � 3
    ...
    if (isSplitted)
      r.setIdTipologia(3l);
    // se la tipologia � di tipo 3, l'idOperazione e l'importo non sono
    // modificabili, dunque prendo i valori precedenti (se � UNSPLITTED)
    if (r.getIdTipologia() != null && r.getIdTipologia() == 3)
    {
      List<RiduzioniSanzioniDTO> rid = quadroEJB
          .getElencoRiduzioniSanzioni(po.getIdProcedimentoOggetto());
