...
public class CUNEMBO206Monitoraggio extends BaseController
{
      ...
      for (String[] servizio : servizi)
      {
        ...
        if (url != null)
        {
          ...
          if (isTst || url.contains("test"))
          {
            if (isTst)
            {
              return "Il pacchetto � stato prodotto per l'ambiente di PRODUZIONE ma � stata rilevata la presenza sospetta della stringa \"tst-\" (che in genere indica i server di test) in almeno uno dei puntamenti dei servizi, potrebbe esserci un problema... N.B. Non � necessariamente un errore ma si prega di verificare";
            }
            else
            {
              return "Il pacchetto � stato prodotto per l'ambiente di PRODUZIONE ma � stata rilevata la presenza sospetta della stringa \"test\" (che in genere indica i server di test) in almeno uno dei puntamenti dei servizi, potrebbe esserci un problema... N.B. Non � necessariamente un errore ma si prega di verificare";
            }
          }
          if (url.contains("coll-"))
          {
            return "Il pacchetto � stato prodotto per l'ambiente di PRODUZIONE ma � stata rilevata la presenza sospetta della stringa \"coll-\" (che in genere indica i server di collaudo) in almeno uno dei puntamenti dei servizi, potrebbe esserci un problema... N.B. Non � necessariamente un errore ma si prega di verificare";
          }
        }
      }
    }
    ...
    try
    {
      for (String[] servizio : servizi)
      {
        ...
        if (url != null)
        {
          ...
          if (url.contains("tst-"))
          {
            return "Il pacchetto � stato prodotto per l'ambiente di COLLAUDO ma � stata rilevata la presenza sospetta della stringa \"tst-\" (che in genere indica i server di test) in almeno uno dei puntamenti dei servizi, potrebbe esserci un problema... N.B. Non � necessariamente un errore ma si prega di verificare";
          }
        }
      }
    }
  ...
  private String[] checkWSMessaggistica(UtenteAbilitazioni utenteAbilitazioni)
  {
    ...
    {
      result[1] = IMAGE_SUCCESS; // LogoutException non � un malfunzionamento
                                 // del servizio
      return result;
    }
    catch (Exception e)
