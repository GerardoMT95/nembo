package it.csi.nembo.nembopratiche.presentation.interceptor.logout;
...
import it.csi.nembo.nembopratiche.integration.ws.papuaserv.messaggistica.Messaggio;
...
public class MessaggisticaManager extends BaseManager
{
    ...
    try
    {
      Map<String, String> mapParametri = NemboUtils.APPLICATION
          .getEjbQuadro().getParametriComune(
              NemboConstants.PARAMETRO.MESSAGGISTICA_MINUTI_VISUALIZZAZIONE_TESTATA,
              NemboConstants.PARAMETRO.MESSAGGISTICA_MINUTI_ATTESA_VERIFICA_LOGOUT,
              NemboConstants.PARAMETRO.MESSAGGISTICA_MINUTI_ATTESA_VERIFICA_REFRESH);
      Long minutiVisualizzazioneMessaggi = NemboUtils.NUMBERS
      ...
      Long minutiVerificaLogout = NemboUtils.NUMBERS
          .getNumericValue(mapParametri.get(
              NemboConstants.PARAMETRO.MESSAGGISTICA_MINUTI_ATTESA_VERIFICA_LOGOUT));
      ...
      Long minutiVerificaNuoviMessaggi = NemboUtils.NUMBERS
          .getNumericValue(mapParametri.get(
              NemboConstants.PARAMETRO.MESSAGGISTICA_MINUTI_ATTESA_VERIFICA_REFRESH));
      ...
      HttpServletResponse response, Object handler) throws Exception
  {
    ...
    {
      /*
       * Non c'� in sessione l'hashmap della messaggista ==> Non si � passati
       * per la pagina della messaggistica di login ==> Non � il percorso
       ...
       * RITORNA AL LOGIN!
       */
      response.sendRedirect("/" + NemboConstants.NEMBOPRATICHE.WEB_CONTEXT
          + "/cunembo201/index.do");
    ...
    {
      /*
       * Se il metodo checkMessaggistica ha restituito una stringa !=null �
       * perch� � in corso un logout forzato, quindi redirigo sulla pagina di
       * logout dove verr� visualizzato il messaggio restituito dal servizio
       * (passato come cookie)
       */
      ...
      Map<String, Object> mapMessaggistica)
  {
    ...
    {
      // Se eseguo un refresh dovr� aggiornare i dati in sessione
      // (indipendentemente che sia andato bene o male)
      refreshSession = true;
      try
      {
        final IMessaggisticaWS messaggisticaWS = NemboUtils.WS
    ...
    else
    {
      if (deltaTimestamp > MINUTI_VERIFICA_LOGOUT * 60 * 1000)
      {
        ...
        {
          // Se eseguo un logout dovr� aggiornare i dati in sessione
          refreshSession = true;
          return performLogout(session, e);
        }
        catch (Exception e)
