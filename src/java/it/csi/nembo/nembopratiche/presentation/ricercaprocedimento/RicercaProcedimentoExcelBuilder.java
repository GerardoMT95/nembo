      ...
      throws Exception
  {
    ...
    for (ProcedimentoOggettoVO item : elenco)
    {
      ...
      aRow.createCell(countCol).setCellValue(
          NemboUtils.DATE.parseDate(item.getDataUltimoAggiornamento()));
       ...
       */
      countCol++;
      aRow.createCell(countCol).setCellValue(NemboUtils.NUMBERS
      ...
      countCol++;
      aRow.createCell(countCol).setCellValue(
          NemboUtils.NUMBERS.nvl(item.getImportoLiquidato()).doubleValue());
      aRow.getCell(countCol).setCellStyle(styleValNumber);
