...
public class LinkTag extends BaseTag
{
  ...
  public int doEndTag() throws JspException
  {
    ...
    // Se il caso d'uso � di scrittura ma l'utente � di sola lettura allora esco
    if (readWrite.booleanValue()
        && NemboUtils.PAPUASERV.isUtenteReadOnly(utenteAbilitazioni))
      return super.doEndTag();
    ElencoCduDTO cdu = NemboUtils.APPLICATION.getCdu(useCase);
    if (forceVisualization || (cdu != null && NemboUtils.PAPUASERV
        .isMacroCUAbilitato(utenteAbilitazioni, cdu.getExtCodMacroCdu())))
    {
      ...
      buffer.append("<div class=\"titledetail toggle_target_" + useCase
          ...
          + NemboUtils.STRING.nvl(this.description).replace("\n", "<br />")
          + "</p></div>");
      try
      {
