...
package it.csi.nembo.nembopratiche.presentation.operazioniprocedimento.documentispesa;
...
@Controller
@RequestMapping("/cunembo263m")
@NemboSecurity(value = "CU-NEMBO-263-M", controllo = NemboSecurity.Controllo.PROCEDIMENTO)
public class CUNEMBO263ModificaDocumentiSpesa
    extends DocumentiSpesaBaseController
{
      ...
      throws InternalUnexpectedException, ApplicationException
  {
    ...
    if ("K".equals(tipoDocumentoSpesa.getFlagIdFornitore())
      ...
      {
        throw new ApplicationException(
            "Impossibile procedere con l'associazione degli interventi in quanto non � stato specificato il fornitore per questo documento!");
      }
      ...
      HttpServletRequest request) throws InternalUnexpectedException
  {
    ...
    // verifico Se ho gi� rendicontato un intervento con il documento di
        ...
        && !elencoInterventiSelezionati.isEmpty())
    {
      for (DecodificaDualLIstDTO<Long> decInt : elencoInterventiSelezionati)
      {
        ...
        if (!trovato)
        {
          // se si pu� togliere lo devo togliere dal db
          trovato = false;
          for (long idDocSpesa : idsDocSpesa)
          {
                ...
                && importoRendicontato.compareTo(BigDecimal.ZERO) > 0)
            {
              ...
              model.addAttribute("msgErrore",
                  "Non � possibile eliminare l'intervento "
                      + interventoDett.getDescIntervento()
                      + " in quanto � gi� stato rendicontato con documento spesa!");
              model.addAttribute("idProcedimento",
                  getProcedimentoFromSession(session).getIdProcedimento());
              return "documentispesa/modificainterventi";
            }
          ...
          .getElencoDocumenti())
      {
        ...
        if (listRic != null)
          for (RicevutaPagamentoVO r : listRic)
          {
            ...
            // se l'importo � null o zero e il flag rendicont con
            // iva � S
                    ...
                    .getFlagRendicontazioneConIva()))
            {
              ...
              // sommatoria NEMBO_R_DOC_SPESA_INT_RICEV_PA.IMPORTO
    ...
    if (idsInterventi == null)
      return "redirect:../cunembo263l/index.do";
      ...
      HttpServletRequest request) throws InternalUnexpectedException
  {
      ...
      if (idsInterventi != null)
        for (String id : idsInterventi)
        {
          ...
          if (importoBd != null)
          {
            ...
            BigDecimal importoRendicontato = NemboUtils.NUMBERS.nvl(
            ...
            if (isRendicontatoSuperato)
            {
              errors.addError("importo_" + idUnivoco,
                  "La somma degli importi sugli interventi del documento "
                      ...
                      + " deve essere maggiore o uguale all'importo rendicontato che � pari a "
                      + NemboUtils.FORMAT
      ...
      switch (importoMax.compareTo(importoBdTotale))
      {
        ...
        case -1:
        {
          errore = " La somma degli importi sugli interventi del documento "
              + documentoSpesaVO.getDescrTipoDocumento()
              + " deve essere minore o uguale all'importo documento che � pari a "
    ...
    for (DettaglioInterventoDTO intervento : listaInterventi)
          ...
          .getElencoDocumenti())
      {
        ...
        if (sommaImpDaAssociare != null && importoBd != null)
          ...
          {
            errors.addError("importo_" + idUnivoco,
                "La somma degli importi da associare deve essere minore o uguale all'importo da rendicontare che � pari a "
            ...
            if (tot.compareTo(r.getImportoPagamento()) > 0)
            {
              for (String idU : idsUnivocoError)
              {
                errors.addError("importoRicevuta_" + idU,
                    "La somma degli importi da associare per la stessa ricevuta non pu� superare l'importo totale della ricevuta che � pari a "
                        + r.getImportoPagamentoStr());
              }
              // errore += "La somma degli importi da associare per la stessa
              // ricevuta non pu� superare l'importo totale della ricevuta.
              // <br>";
            }
          }
          ...
          if (sommaImpDaAssociare.compareTo(doc.getImportoRendicontato()) < 0)
          {
            for (RicevutaPagamentoVO r : doc.getElencoRicevutePagamento())
            {
              errors.addError("importoRicevuta_"
                  ...
                  + doc.getIdDocumentoSpesa() + "_"
                  + r.getIdDettRicevutaPagamento(),
                  "La somma degli importi da associare deve essere maggiore o uguale all'importo rendicontato che � pari a "
    ...
    if (!errors.isEmpty())
    {
      StringBuilder errorMsg = new StringBuilder(
          "Sono stati rilevati degli errori nei dati inseriti. I campi errati sono stati contrassegnati in rosso, � necessario correggerli per proseguire");
      ...
      throws InternalUnexpectedException
  {
    setModelDialogWarning(model,
        ...
        "../cunembo263m/confermaimportipopup.do");
  ...
  @NemboSecurity(value = "CU-NEMBO-263-L", controllo = NemboSecurity.Controllo.PROCEDIMENTO)
      ...
      throws InternalUnexpectedException
  {
    ...
    try
    {
      ...
      quadroEJB.insertOrUpdateFileAllegatoDocSpesaGiaRendicontato(d,
          NemboUtils.PAPUASERV.getFirstCodiceAttore(
    ...
    return "redirect:../cunembo263m/elencoricevuteDocRendicontato_" + idDocSpesa
        + ".do";
  }
