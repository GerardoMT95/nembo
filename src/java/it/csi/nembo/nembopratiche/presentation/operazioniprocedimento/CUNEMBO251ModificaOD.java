...
@Controller
@NemboSecurity(value = "CU-NEMBO-251", controllo = NemboSecurity.Controllo.DEFAULT)
@RequestMapping(value = "/cunembo251")
public class CUNEMBO251ModificaOD extends BaseController
{
  @Autowired
    ...
    if (elencoUfficiZona != null)
    {
      for (DecodificaDTO<String> ufficio : elencoUfficiZona)
      {
        htmlOptions.append("<option title=\""
            + NemboUtils.STRING.safeHTMLText(ufficio.getDescrizione())
            + "\" value=\"" + ufficio.getId()
            + "\">"
            + NemboUtils.STRING.safeHTMLText(ufficio.getDescrizione())
            + "</option>");
      ...
      throws InternalUnexpectedException
  {
    ...
    if (elencoTecnici != null)
    {
      ...
      {
        htmlOptions.append("<option title=\""
            + NemboUtils.STRING.safeHTMLText(tecnico.getDescrizione())
            + "\" value=\"" + tecnico.getId()
            + "\">"
            + NemboUtils.STRING.safeHTMLText(tecnico.getDescrizione())
            + "</option>");
      ...
      throws InternalUnexpectedException
  {
    ...
    if (!GenericValidator.isBlankOrNull(note))
    {
      ...
      {
        return "Il campo \"Note\" pu� contenere al massimo 4000 caratteri.";
      }
    }
    ...
    // se la modifica non � massiva la lista � vuota -> aggiunto il procedimento
    // corrente alla lista
    ...
    if (errors.isEmpty())
    {
      ...
      if (delegatoDTO != null)
      {
        ...
        if (allProcHaveSameUff)
        {
          ...
          {
            /*
             * se anche il tecnico � uguale e non cambia, allora segnalo
             * l'errore
             */
    ...
    if (errors.isEmpty())
    {
                  ...
                  idProcedimentiSelezionati.elementAt(0))))
      {
        ...
        if (!isMassivo)
          return "redirect:../cunembo129/index_"
  ...
  private String checkInfo(ArrayList<Long> vect) throws InternalUnexpectedException
  {
    ...
    if (!checkStatoOggetto)
      return "La modifica non pu� essere effettuata in base allo stato del procedimento.";
