...
import it.csi.nembo.nembopratiche.dto.ExcelRigaDomandaDTO;
...
@Controller
@RequestMapping("/cunembo263l")
@NemboSecurity(value = "CU-NEMBO-263-L", controllo = NemboSecurity.Controllo.PROCEDIMENTO)
public class CUNEMBO263lElencoDocumentiSpesa extends BaseController
{
  @Autowired
      ...
      throws InternalUnexpectedException, ApplicationException
  {
    ...
    /*
     * La funzionalit� � presente solo se
     * NEMBO_D_BANDO.RENDICONTAZIONE_DOC_SPESA = �S�. Consento l'accesso alla
     * funzionalit� solo se non � gi� stata fatta una rendicontazione prima
    ...
    if (!quadroEJB.canUseDocumentiSpesa(
      ...
      throw new ApplicationException(
          "Per la pratica selezionata non � possibile procedere poich� esiste una rendicontazione, effettuata senza i documenti di spesa, precedente all'abilitazione della funzionalit� richiesta.");
    /*
     * Nel caso particolare in cui risultino associati al procedimento solo
     * interventi per cui non � prevista la gestione dei documenti di spesa (e
     * cio� che hanno NEMBO_R_LIVELLO_INTERVENTO.FLAG_DOCUMENTO_SPESA = �N�),
    ...
    if (0 == quadroEJB.contaInterventiRendicontazConFlagS(
        ...
        getProcedimentoFromSession(session).getIdBando()))
      throw new ApplicationException(
          "Per la pratica selezionata non � possibile procedere poich� non ci sono interventi in domanda per cui � prevista la rendicontazione mediante documenti di spesa.");
    ...
    String hasHelp = nuovoProcedimento.getHelpCdu("CU-NEMBO-263-L", null);
    if (hasHelp != null)
      ...
      throws InternalUnexpectedException, ApplicationException
  {
    ...
    NemboFactory.setIdProcedimentoInSession(session, idProcediemnto);
        ...
        for (DecodificaDualLIstDTO<Long> item : elencoInterventiSelezionati)
        {
          ...
          if (elencoDocumenti != null && !elencoDocumenti.isEmpty())
            for (DocumentoSpesaVO doc : elencoDocumenti)
            {
              ...
              if (doc.getNomeFileFisicoDocumentoSpe() != null)
              {
                String icon = NemboUtils.FILE.getDocumentCSSIconClass(
