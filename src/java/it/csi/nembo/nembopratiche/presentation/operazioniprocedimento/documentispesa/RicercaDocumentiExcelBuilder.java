...
public class RicercaDocumentiExcelBuilder extends AbstractExcelView
{
      ...
      throws Exception
  {
    ...
    String idProcediemnto = NemboFactory.getProcedimento(request)
        .getIdentificativo();
    ...
    if (elenco != null)
    {
      ...
      for (ExcelRicevutePagInterventoDTO intervento : elenco)
      {
        ...
        nInt++; // incremento ad ogni intervento -> quando � uguale a nInterv ->
                // � l'ultimo record dello stesso intervento, quindi poi faccio
                // i totali
            ...
            .getDettaglioDocumento())
        {
          ...
          for (ExcelRigaRicevutaPagDTO ricevuta : documento.getRicevute())
          {
            totaleRicevuteBd = totaleRicevuteBd
                .add(NemboUtils.NUMBERS.nvl(ricevuta.getImportoPagamento()));
            totaleRicevuteAssBd = totaleRicevuteAssBd.add(NemboUtils.NUMBERS
            ...
            if (countricevute == 0)
            {
              ...
              countCol = 1;
              aRow.createCell(countCol).setCellValue(NemboUtils.FORMAT
          ...
          aRow.createCell(8).setCellValue(
              NemboUtils.FORMAT.formatCurrency(totaleRicevuteBd));
          aRow.getCell(8).setCellStyle(styleValTotaleParziale);
          aRow.createCell(9).setCellValue(
              NemboUtils.FORMAT.formatCurrency(totaleRicevuteAssBd));
        ...
        if (nInt == nInterv)
        {
          ...
          colTot++;
          aRow.createCell(colTot).setCellValue(NemboUtils.FORMAT
          ...
          colTot++;
          aRow.createCell(colTot).setCellValue(
              NemboUtils.FORMAT.formatCurrency(totaleRicevuteDocumentiBd));
          aRow.getCell(colTot).setCellStyle(styleValTotale);
          colTot++;
          aRow.createCell(colTot).setCellValue(NemboUtils.FORMAT
    ...
    if (elencoRicevute != null)
    {
      for (ExcelRigaDocumentoSpesaDTO documento : elencoRicevute)
      {
        ...
        for (ExcelRigaRicevutaPagDTO ricevuta : documento.getRicevute())
        {
          ...
          if (countricevute == 0)
          {
            ...
            aRow.createCell(countCol).setCellValue(
                NemboUtils.FORMAT.formatCurrency(documento.getImportoIva()));
            aRow.getCell(countCol).setCellStyle(styleValNumber);
            cellRangeAddress = new CellRangeAddress(rowCount,
                rowCount + totaleRicevute - 1, countCol, countCol);
            HSSFRegionUtil.setBorderTop(CellStyle.BORDER_THIN, cellRangeAddress,
        ...
        aRow.createCell(14).setCellValue(
            NemboUtils.FORMAT.formatCurrency(totaleRicevuteAssociateBd));
