...
@Controller
@RequestMapping("/cunembo263e")
@NemboSecurity(value = "CU-NEMBO-263-E", controllo = NemboSecurity.Controllo.PROCEDIMENTO)
public class CUNEMBO263EliminaDocumentiSpesa extends BaseController
{
  @Autowired
  IQuadroEJB              quadroEJB           = null;

  protected static String COMMON_SESSION_NAME = "CU-NEMBO-263";
      ...
      throws InternalUnexpectedException
  {
    ...
    if (!quadroEJB.canDeleteDocSpesa(idDocumentoSpesa))
      return "Impossibile eliminare. Documento gi� associato ad una domanda.";
      ...
      throws InternalUnexpectedException
  {
    ...
    if (quadroEJB.docHasInterventi(idDocSpesa))
    {
      common.put("msgElimina",
          "Il documento � gi� stato associato almeno in parte ad uno o pi� interventi. Se si conferma l'operazione saranno eliminate anche queste associazioni. Si desidera proseguire?");
      saveCommonInSession(common, request.getSession());
      return "dialog/success";
    }
    ...
    {
      common.put("msgElimina",
          "Il documento � gi� stato associato ad una o pi� ricevute di pagamento. Se si conferma l'operazione saranno eliminate anche queste associazioni. Si desidera proseguire?");
      saveCommonInSession(common, request.getSession());
      return "dialog/success";
    }
      ...
      throws InternalUnexpectedException
  {
    Map<String, Object> common = getCommonFromSession(COMMON_SESSION_NAME,
        request.getSession(), false);
    // ricevuta gia usata per rendicontazione (NEMBO_R_DOC_SPESA_PROC_OGG )
    ...
    {
      return "La ricevuta di pagamento � gi� stata utilizzata per la rendicontazione spese, per cui non � pi� possibile eliminarla.";
    }

    if (quadroEJB.ricEsisteInDocSpesIntRicPag(idRicevutaPagamento))
    {
      common.put("msgElimina",
          "La ricevuta di pagamento � gi� stata associata almeno in parte ad uno o pi� interventi. Se si conferma l'operazione saranno eliminate anche queste associazioni. Si desidera proseguire?");
      return "<success>";
    }
