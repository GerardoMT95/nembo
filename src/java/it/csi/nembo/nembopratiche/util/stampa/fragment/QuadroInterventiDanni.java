			...
			String cuName) throws Exception
	{
	    ...
	    List<RigaElencoInterventi> elenco = ejbInterventi
	            .getElencoInterventiProcedimentoOggetto(idProcedimentoOggetto,
	                NemboConstants.FLAGS.NO, procedimentoOggetto.getDataFine());
		...
		if (elenco != null && elenco.isNotEmpty())
		{
			for (RigaElencoInterventi item : elenco)
			{
				...
				writeTag(writer, TAG_NAME_PROGRESSIVO_DANNO, NemboUtils.STRING.nvl(item.getProgressivoDanno()));
		...
		if(listParticellare != null && listParticellare.size()>0)
		{
			...
			for(DatiLocalizzazioneParticellarePerStampa particella: listParticellare)
			{
				...
				writeTag(writer,TAG_NAME_PROGRESSIVO,NemboUtils.STRING.nvl(particella.getProgressivo()));
				writeTag(writer,TAG_NAME_COMUNE,particella.getDescComune());
				writeTag(writer,TAG_NAME_SEZIONE,particella.getSezione());
				writeTag(writer,TAG_NAME_FOGLIO,NemboUtils.STRING.nvl(particella.getFoglio()));
				writeTag(writer,TAG_NAME_PARTICELLA,NemboUtils.STRING.nvl(particella.getParticella()));
				writeTag(writer,TAG_NAME_SUBALTERNO,NemboUtils.STRING.nvl(particella.getSubalterno()));
